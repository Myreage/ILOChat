...
runserver : all
	$(TERM)  -title server -e $(JAVA) -classpath $(BIN):$(CLASSPATH) RunChatServer --verbose --noquit &

# Lancement d'un client console
runclient : all
	$(TERM) -title "Zebulon" -e $(JAVA) -classpath $(BIN):$(CLASSPATH) RunChatClient --name Zébulon --verbose &

# Lancement d'un client graphique version 1
rungui1 : all
	$(JAVA) -classpath $(BIN):$(CLASSPATH) RunChatClient --name Ténéphore --gui 1 --verbose

# Lancement d'un client graphique version 2
rungui2 : all
	$(JAVA) -classpath $(BIN):$(CLASSPATH) RunChatClient --name Zéphirine --gui 2 --verbose
...
rundemo : all
	...
	sleep 1;
	$(TERM) -title "Zebulon" -e $(JAVA) -classpath $(BIN):$(CLASSPATH) RunChatClient --name Zebulon & \
	$(JAVA) -classpath $(BIN):$(CLASSPATH) RunChatClient --name Tenephore --gui 2;
